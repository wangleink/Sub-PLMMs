...
ALLresult <- foreach(run=1:nsim, .combine=abind::abind, .multicombine=TRUE, .init=array(0,c(nrow=240,ncol=p,ndim=0))) %dopar% {
  ...
  pi2.total.linear=sqrt((Y.init-(XZ)%*%tmp.total.linear)^2*rowSums(t(Phi.total.init.ginv%*%t(XZ))^2))
...
result1=as.matrix(rbind(cbind(Direct.Opt[,1],Direct.Unif[,1]),
                        cbind(Direct.Opt[,2],Direct.Unif[,2]),
                        cbind(Direct.Opt[,3],Direct.Unif[,3]),
                        cbind(Direct.Opt[,4],Direct.Unif[,4])))

colnames(result1)=c("Direcrt:Opt","Unif")
...
result2=as.matrix(rbind(cbind(Total.Opt[,1],Total.Unif[,1]),
                        cbind(Total.Opt[,2],Total.Unif[,2]),
                        cbind(Total.Opt[,3],Total.Unif[,3]),
                        cbind(Total.Opt[,4],Total.Unif[,4])))

colnames(result2)=c("Total:Opt","Unif")
rownames(result2)=c("r400,bias","mse","emse","sd","se","CP",
                    "r500,bias","mse","emse","sd","se","CP",
                    "r600,bias","mse","emse","sd","se","CP",
                    "r700,bias","mse","emse","sd","se","CP")

result3=as.matrix(rbind(cbind(Indirect.Opt[,1],Indirect.Unif[,1]),
                        cbind(Indirect.Opt[,2],Indirect.Unif[,2]),
                        cbind(Indirect.Opt[,3],Indirect.Unif[,3]),
                        cbind(Indirect.Opt[,4],Indirect.Unif[,4])))

colnames(result3)=c("Indirect:Opt","Unif")
